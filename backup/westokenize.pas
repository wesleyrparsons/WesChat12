...
begin
  ...
  if UDTagON and (Length(SymbolTable) < UDTagBoundary) then
    raise Exception.CreateFmt(
      'UDTag is enabled, but SymbolTable has only %d entries; at least %d are required.',
      [Length(SymbolTable), UDTagBoundary]);
  ...
  while i < Length(Corpus) do begin
    ...
    if UDTagOn and (Corpus[i] = Ord('|')) then begin
